...
void dovo_destination::OnNameUpdate( wxKeyEvent& event )
{
	...
	if(sel != -1)
	{
		wxString a = m_name->GetValue();
		if(a.length() > 0) 
		{
			m_destinations[sel].name = a;		
		...
		else
		{
			...
			m_name->SetValue(m_destinations[sel].name);
		}
...
void  dovo_destination::SetCtrlState()
{
	...
	else
	{
		...
		m_destinationHost->SetValue(m_destinations[sel].destinationHost);
		...
		m_destinationAETitle->SetValue(m_destinations[sel].destinationAETitle);
		m_ourAETitle->Enable(true);
		m_ourAETitle->SetValue(m_destinations[sel].ourAETitle);
	}
}
...
void dovo_destination::UpdateItem(int sel)
{
	...
	m_destinations[sel].name = m_name->GetValue();
	m_destinations[sel].destinationHost = m_destinationHost->GetValue();
	m_destinations[sel].destinationPort = boost::lexical_cast<int>(m_port);
	m_destinations[sel].destinationAETitle = m_destinationAETitle->GetValue();
	m_destinations[sel].ourAETitle = m_ourAETitle->GetValue();
}